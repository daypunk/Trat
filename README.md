...
Trat은 ML Kit을 활용한 **오프라인 실시간 번역 앱**입니다. 네트워크 연결 없이도 한국어, 영어, 일본어, 중국어 간의 양방향 번역을 제공하며, 채팅 형태의 직관적인 UI로 번역 결과를 관리할 수 있습니다.
...
- **현대적 UI**: Toss 디자인 스타일, 다크모드, Lottie 애니메이션

## 기술 스택 및 아키텍처

### 아키텍처 패턴: Clean Architecture + MVVM

이 프로젝트는 **Clean Architecture**와 **MVVM(Model-View-ViewModel)** 패턴을 결합하여 구현되었습니다.
...
- **ViewModel & LiveData**: MVVM 패턴 구현
- **Navigation Component**: 화면 전환 관리

**번역 & AI**
- **ML Kit Translate**: Google의 온디바이스 번역 모델
- **언어 감지**: 자동 입력 언어 판별

**성능 최적화**
- **Coroutines & Flow**: 비동기 처리 및 반응형 프로그래밍
- **DataStore**: 고성능 캐싱 시스템
- **DB 인덱싱**: 검색 성능 최적화
...
이 프로젝트를 통해 다음과 같은 현대적 안드로이드 개발 기술을 학습할 수 있습니다:
...
Trat은 **현대적인 안드로이드 앱 개발의 모범 사례**를 보여주는 프로젝트입니다. Clean Architecture와 MVVM 패턴을 올바르게 적용하여 확장 가능하고 유지보수하기 쉬운 코드베이스를 구축했습니다. 

각 계층의 명확한 역할 분담, 의존성 주입을 통한 느슨한 결합, 그리고 성능 최적화까지 고려한 **엔터프라이즈급 앱 아키텍처**를 구현했다고 자신있게 말할 수 있습니다.
